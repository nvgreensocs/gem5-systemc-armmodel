...
{
public:
	amba::amba_slave_socket<> slave_socket;
...
private:
	//unsigned char * MEM;
        amba::amba_burst_size  default_burstsize;
...
template<unsigned int BUSWIDTH>
AHB_simple_slave<BUSWIDTH>::AHB_simple_slave(sc_core::sc_module_name nm, std::string debug_port_name, std::string slave_port_name):sc_core::sc_module(nm),
slave_socket(slave_port_name.c_str(),amba::amba_AXI,amba::amba_CT,false),
...
m_data_pending(false)
{
		...
		slave_socket.template register_nb_transport_fw< AHB_simple_slave<BUSWIDTH> >(this, & AHB_simple_slave<BUSWIDTH>::nb_fw_transport);
...
{
	slave_socket.set_timing_listener_callback(this, & AHB_simple_slave::masterTimingListener);
	slave_socket.activate_synchronization_protection();
}

template<unsigned int BUSWIDTH>
void AHB_simple_slave<BUSWIDTH>::responseProcess()
{
	...
	if(current_trans!=NULL)
	{
		...
		else
		{
			...
			{
				unsigned char *data= current_trans->get_data_ptr();
				tlm::tlm_phase ph = amba::BEGIN_LAST_RESP;
				msg.str("");
				for(unsigned int j=0; j<size->value; j++)
				{
                                        memory->read(txn_address, 1, (unsigned char*)(data+(4*counter)+j));
...
tlm::tlm_sync_enum AHB_simple_slave<BUSWIDTH>::nb_fw_transport(tlm::tlm_generic_payload & trans, tlm::tlm_phase & ph, sc_core::sc_time & delay)
{
	...
	if(ph==tlm::BEGIN_REQ)
	{
		...
		//current_trans->template get_extension<amba::amba_burst_size>(size);
                if (!slave_socket.template get_extension<amba::amba_burst_size>(size, trans))
                    size=&default_burstsize;
