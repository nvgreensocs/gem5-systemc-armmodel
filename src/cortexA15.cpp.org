...
void CortexA15::empty_requests()
{
	if ((_current_request == NULL) && (_pending_requests.size() > 0) && (_system_cfg == "bareMetal") )
	{
		...
		m_Api->getValue<std::string> ("Tasks." + _current_request->task_name + ".debug",debug_gem5);
		...
		{
			sysPars->kernel=m_Api->getValue("Tasks." + _current_request->task_name + ".kernel");
...
void CortexA15::checkpoint(const string &cpt_dir)
{
	curTick(sc_core::sc_time_stamp().value());
